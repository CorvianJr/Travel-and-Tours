$(document).ready(function() {
    ...
    function addPriceCard(priceCode = '', currency = '', price = '', priceDesc = '') {
        ...
        var priceCardHtml= `
            <div id="priceCard${priceCount}" class="price-card-add rounded bg-dark p-2 mb-2">
                <h5>Option ${priceCount}</h5>
                <div class="row">
                    <div class="col-md-6"> 
    ...
    function addFlightCard(flightCode = '', plane = '', start = '', end = '') {
        var flightCount = $('.flight-card-add').length + 1;
        var flightCardHtml = `
            <div id="flightCard${flightCount}" class="flight-card-add rounded bg-dark p-2 mb-2">
                <h5>Flight ${flightCount}</h5>
                <div class="row">
                    <div class="mb-2">
                        <div class="form-floating">
                            <input class="form-control" type="text" id="plane${flightCount}" placeholder="" value="${plane}" readonly>
                ...
                <div class="row d-flex justify-content-center">
                    <div class="col-md-4  w-50">
                        <div class="form-floating mb-2">
                            <input class="form-control" type="date" id="schedStart${flightCount}" value="${start}" readonly>
                            <label for="schedStart${flightCount}">Start Date</label>
                        </div>
                    </div>
                    <div class="col-md-4  w-50">
                        <div class="form-floating mb-2">
                            <input class="form-control" type="date" id="schedEnd${flightCount}" placeholder="" value="${end}" readonly>
    ...
    function fetchFlightData(packCode) {
        $.ajax({
            ...
            success: function(data) {
                ...
                for (var i = 0; i < data.length; i++) {
                    addFlightCard(data[i]['flight_code'], data[i]['plane'], data[i]['travel_start'], data[i]['travel_end']);
                }
            },
            error: function(xhr, status, error) {
                // Log any errors that occur during the AJAX request
